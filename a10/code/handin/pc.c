...
uthread_mutex_t mx;
uthread_cond_t need_item;
uthread_cond_t have_item;
...
void* producer (void* v) {
  ...
  for (int i=0; i<NUM_ITERATIONS; i++) {
    items += 1;
    uthread_cond_signal(have_item);
    while(items >= MAX_ITEMS){
      uthread_cond_wait(need_item);
    }
...
void* consumer (void* v) {
  ...
  for (int i=0; i<NUM_ITERATIONS; i++) {
    while (items == 0){
      uthread_cond_wait(have_item);
    }
    items -= 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;  
    uthread_cond_signal(need_item);
  }
  uthread_mutex_unlock(mx);
  return NULL;
}
...
int main (int argc, char** argv) {
  ...
  need_item = uthread_cond_create(mx);
  have_item = uthread_cond_create(mx);
